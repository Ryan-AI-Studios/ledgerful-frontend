  ...
  return (
    <MarketingLayout>
      ...
      <section className="border-b border-[var(--color-border-muted)]">
        <div className="max-w-[1080px] mx-auto px-4 md:px-6 pt-20 pb-16 md:pt-28 md:pb-24">
          <div className="max-w-3xl">
            <h1 className="text-[2rem] md:text-[2.5rem] font-semibold leading-[1.1] tracking-[-0.022em] text-[var(--color-text-primary)] [text-wrap:balance]">
              Local-first change intelligence.
              <br />
              <span className="text-[var(--color-primary)]">Cryptographic provenance.</span>
              <br />
              AI governance ready.
            </h1>
            <p className="mt-6 max-w-2xl text-[1.0625rem] leading-relaxed text-[var(--color-text-secondary)] [text-wrap:pretty]">
              The intent ledger for high-integrity engineering. Where git answers{" "}
              <em className="not-italic text-[var(--color-text-primary)]">what changed</em>, Ledgerful answers{" "}
              <em className="not-italic text-[var(--color-text-primary)]">why it changed, who decided, and whether the impact was understood.</em>
            </p>
            <div id="install" className="mt-8 flex flex-col gap-4 scroll-mt-24">
              <CopyCommand command="npm install -g @ledgerful/cli" />
              <Link
                href="/pricing"
                className="inline-flex items-center gap-1.5 text-sm font-semibold text-[var(--color-text-secondary)] hover:text-[var(--color-primary)] transition-colors duration-150"
              >
                View pricing <ArrowRight className="w-4 h-4" aria-hidden="true" />
              </Link>
            </div>
            <p className="mt-3 text-xs text-[var(--color-text-muted)]">
              Runs entirely on your machine. No SaaS, no telemetry, no data egress.
            </p>
          </div>
        </div>
      </section>

      {/* Terminal receipt — the evidence */}
      <section className="border-b border-[var(--color-border-muted)] bg-[var(--color-surface-alt)]">
        <div className="max-w-[1080px] mx-auto px-4 md:px-6 py-16 md:py-20 grid lg:grid-cols-[1fr_1.1fr] gap-10 lg:gap-14 items-center">
          <div>
            <p className="text-[0.6875rem] font-semibold uppercase tracking-wider text-[var(--color-primary)] mb-3">
            ...
            <h2 className="text-[1.5rem] font-semibold tracking-[-0.015em] text-[var(--color-text-primary)] [text-wrap:balance]">
              Risk you can screenshot and trust.
            </h2>
            <p className="mt-4 text-[0.9375rem] leading-relaxed text-[var(--color-text-secondary)] [text-wrap:pretty]">
              A manager posts the scan output in Slack. A staff engineer reads the couplings without opening the diff.
              Risk is color, icon, and text — never one alone. This is the CLI, rendered for the people who approve the work.
            </p>
          </div>
          <TerminalReceipt />
        </div>
      ...
      <section className="border-b border-[var(--color-border-muted)]">
        <div className="max-w-[1080px] mx-auto px-4 md:px-6 py-16 md:py-20">
          ...
          <ol className="grid md:grid-cols-3 gap-px bg-[var(--color-border-muted)] rounded-lg overflow-hidden border border-[var(--color-border-muted)]">
            {QUESTIONS.map((item) => (
              <li key={item.n} className="bg-[var(--color-surface-alt)] p-6">
                <span className="font-mono text-[0.6875rem] text-[var(--color-text-muted)] block mb-1">{item.n}</span>
                <h3 className="text-[1.0625rem] font-semibold text-[var(--color-text-primary)]">{item.q}</h3>
                <p className="mt-2 text-[0.8125rem] leading-relaxed text-[var(--color-text-secondary)]">{item.a}</p>
              </li>
            ))}
          </ol>
        </div>
      ...
      <section className="border-b border-[var(--color-border-muted)] bg-[var(--color-surface-alt)]">
        <div className="max-w-[1080px] mx-auto px-4 md:px-6 py-16 md:py-20 grid lg:grid-cols-[1.1fr_1fr] gap-10 lg:gap-14 items-center">
          ...
          <div>
            ...
            <p className="mt-4 text-[0.9375rem] leading-relaxed text-[var(--color-text-secondary)] [text-wrap:pretty]">
              Every transaction carries an Ed25519 signature, an intent record, and a category.
      ...
      <section className="border-b border-[var(--color-border-muted)]">
        <div className="max-w-[1080px] mx-auto px-4 md:px-6 py-16 md:py-20">
          ...
          <ol className="grid md:grid-cols-3 gap-6">
            {CHANNELS.map((c, i) => (
              <li key={c.title} className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-alt)] p-6">
      ...
      <section>
        <div className="max-w-[1080px] mx-auto px-4 md:px-6 py-20 md:py-24 text-center">
          <h2 className="text-[1.75rem] md:text-[2rem] font-semibold tracking-[-0.015em] text-[var(--color-text-primary)] [text-wrap:balance]">
            Ready to secure your provenance?
          </h2>
