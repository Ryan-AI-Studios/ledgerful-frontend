...
export async function fetchLedger(): Promise<WithSource<import("@/lib/types").LedgerEntry[]>> {
  return withFallback(() => fetchLiveLedger(), () => fetchMockLedger());
}

export async function fetchLedgerEntry(txId: string): Promise<WithSource<import("@/lib/types").LedgerEntry | undefined>> {
  return withFallback(
